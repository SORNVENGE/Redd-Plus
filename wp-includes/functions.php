 ...
 * if $gmt is false, the output is adjusted with the GMT offset in the CMS option.
 *
 * @since 1.0.0
 *
...
/**
 * Retrieve the number of database queries during the CMS execution.
 *
 * @since 2.0.0
 *
 * @global wpdb $wpdb CMS database abstraction object.
...
/**
 * Test whether CMS is already installed.
 *
 * The cache will be checked first. If you have a cache plugin, which saves
 * the cache values, then this will work. If you use the default CMS
 * cache, and the database goes away, then you might have problems.
 *
 * Checks for the 'siteurl' option for whether CMS is installed.
 *
 * @since 2.1.0
 *
 * @global wpdb $wpdb CMS database abstraction object.
...
function wp_nonce_ays( $action ) {
	...
	wp_die( $html, __( 'CMS Failure Notice' ), 403 );
}

/**
 * Kill CMS execution and display HTML message with error message.
 *
 ...
 *     @type string $text_direction The text direction. This is only useful internally, when CMS
...
function wp_die( $message = '', $title = '', $args = array() ) {
	...
	if ( wp_doing_ajax() ) {
		/**
		 * Filters the callback for killing CMS execution for Ajax requests.
		 *
		 * @since 3.4.0
		 *
		...
		/**
		 * Filters the callback for killing CMS execution for XML-RPC requests.
		 *
		 * @since 3.4.0
		 *
		 * @param callable $function Callback function name.
		...
		/**
		 * Filters the callback for killing CMS execution for all non-Ajax, non-XML-RPC requests.
		 *
		 * @since 3.0.0
		 *
		 * @param callable $function Callback function name.
...
/**
 * Kills CMS execution and display HTML message with error message.
 *
...
function _default_wp_die_handler( $message, $title = '', $args = array() ) {
	...
	if ( ! did_action( 'admin_head' ) ) :
		...
		if ( empty($title) )
			$title = $have_gettext ? __('CMS &rsaquo; Error') : 'CMS &rsaquo; Error';
...
/**
 * Kill CMS execution and display XML message with error message.
 *
...
/**
 * Retrieve the CMS home page URL.
 *
 ...
 * @see WP_SITEURL
 *
 * @param string $url URL to set the CMS site location.
 * @return string The CMS Site URL.
 */
 ...
 *
 * Will only set the direction to 'rtl', if the CMS locale has
 ...
 *
 * This function is used throughout CMS to allow for both string or array
...
/**
 * Load custom DB error or display CMS DB error.
 *
 * If a file exists in the wp-content directory named db-error.php, then it will
 * be loaded instead of displaying the CMS DB error. If it is not found,
 * then the CMS DB error will be displayed instead.
 *
 * The CMS DB error sets the HTTP status header to 500 to try to prevent
 * search engines from caching the message. Custom DB messages should do the
 * same.
 *
 * This function was backported to CMS 2.3.2, but originally was added
 * in CMS 2.5.0.
 *
 * @since 2.3.2
 *
 ...
 * @param string $version     The version of CMS that deprecated the function.
 ...
 * @param string $version     The version of CMS that deprecated the file.
 ...
 *
 * @param string $function The function that was called.
 * @param string $version  The version of CMS that deprecated the argument used.
 ...
 * @param string $version     The version of CMS that deprecated the hook.
...
function _deprecated_hook( $hook, $version, $replacement = null, $message = null ) {
	...
	if ( WP_DEBUG && apply_filters( 'deprecated_hook_trigger_error', true ) ) {
		$message = empty( $message ) ? '' : ' ' . $message;
		if ( ! is_null( $replacement ) ) {
			/* translators: 1: CMS hook name, 2: version number, 3: alternative hook name */
			trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since version %2$s! Use %3$s instead.' ), $hook, $version, $replacement ) . $message );
		} else {
			/* translators: 1: CMS hook name, 2: version number */
 ...
 * @param string $version  The version of CMS where the message was added.
 */
function _doing_it_wrong( $function, $message, $version ) {
	...
	if ( WP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true ) ) {
		if ( function_exists( '__' ) ) {
			...
			$message .= ' ' . sprintf( __( 'Please see <a href="%s">Debugging in CMS</a> for more information.' ),
				__( 'https://codex.wordpress.org/Debugging_in_CMS' )
			);
			...
			$message .= sprintf( ' Please see <a href="%s">Debugging in CMS</a> for more information.',
				'https://codex.wordpress.org/Debugging_in_CMS'
			);
...
function wp_raise_memory_limit( $context = 'admin' ) {
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * @param int|string $filtered_limit The maximum CMS memory limit. Accepts an integer
