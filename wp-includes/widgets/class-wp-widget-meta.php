...
class WP_Widget_Meta extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			...
			'description' => __( 'Login, RSS, &amp; CMS.org links.' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( $title ) {
			...
			<?php
			/**
			 * Filters the "Powered by CMS" text in the Meta widget.
			 *
			 * @since 3.6.0
			 *
			 * @param string $title_text Default title text for the CMS.org link.
			 */
			echo apply_filters( 'widget_meta_poweredby', sprintf( '<li><a href="%s" title="%s">%s</a></li>',
				esc_url( __( 'https://wordpress.org/' ) ),
				esc_attr__( 'Powered by CMS, state-of-the-art semantic personal publishing platform.' ),
				_x( 'CMS.org', 'meta widget link text' )
			) );
