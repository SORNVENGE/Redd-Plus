...
/**
 * CMS Database Access Abstraction Object
 *
...
class wpdb {
	 ...
	 * CMS table prefix
	 *
	 * You can set this to have multiple CMS installations
	 ...
	 * against the required MySQL version for CMS. Normally, a replacement
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		 ...
		 *  - We are a development version of CMS, or
	...
	/**
	 * Change the current SQL mode, and ensure its CMS compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the CMS tables.
	 *
	 * @since 2.5.0
	 *
	...
	/**
	 * Returns an array of CMS tables.
	 *
	 * Also allows for the CUSTOM_USER_TABLE and CUSTOM_USER_META_TABLE to
	 * override the CMS users and usermeta tables that would otherwise
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you don&#8217;t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">CMS Support Forums</a>.' ),
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() ) {
			...
			$error_str = sprintf( __( 'CMS database error %1$s for query %2$s made by %3$s' ), $str, $this->last_query, $caller );
		} else {
			/* translators: 1: Database error message, 2: SQL query */
			$error_str = sprintf( __( 'CMS database error %1$s for query %2$s' ), $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: %s: support forums URL */
				__( 'If you&#8217;re unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">CMS Support Forums</a>.' ),
				__( 'https://wordpress.org/support/' )
			) . "</p>\n";
	...
	public function check_database_version() {
		...
		if ( version_compare($this->db_version(), $required_mysql_version, '<') ) {
			/* translators: 1: CMS version number, 2: Minimum required MySQL version number */
			return new WP_Error('database_version', sprintf( __( '<strong>ERROR</strong>: CMS %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ));
		}
	}
	 ...
	 *
	 * Called when CMS is generating the table scheme.
	 *
