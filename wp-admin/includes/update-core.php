<?php
/**
 * CMS core upgrade functionality.
 *
 * @package CMS
 ...
 * should be installed with the CMS Upgrade. These items will not be
...
/**
 * Upgrades the core of CMS.
 *
 * This will create a .maintenance file at the base of the CMS directory
 ...
 *   2. Create the .maintenance file in current CMS base.
 *   3. Copy new CMS directory over old CMS files.
 *   4. Upgrade CMS to new version.
 *     4.1. Copy all files/folders other than wp-content
 *     4.2. Copy any language files to WP_LANG_DIR (which may differ from WP_CONTENT_DIR
 *     4.3. Copy any new bundled themes/plugins to their respective locations
 *   5. Delete new CMS directory path.
 ...
 * isn't that bad.
 *
 * If the copy of the new CMS over the old fails, then the worse is that
 * the new CMS directory will remain.
 *
 ...
 * @param string $to   Path to old CMS installation.
...
function update_core($from, $to) {
	 ...
	 * the process:
	 *
	 * 1. Before CMS begins the core upgrade process.
	 * 2. Before Maintenance Mode is enabled.
	 * 3. Before CMS begins copying over the necessary files.
	...
	$old_wp_version = $GLOBALS['wp_version']; // The version of CMS we're updating from
	...
	if ( !$mysql_compat && !$php_compat )
		return new WP_Error( 'php_mysql_not_compatible', sprintf( __('The update cannot be installed because CMS %1$s requires PHP version %2$s or higher and MySQL version %3$s or higher. You are running PHP version %4$s and MySQL version %5$s.'), $wp_version, $required_php_version, $required_mysql_version, $php_version, $mysql_version ) );
	elseif ( !$php_compat )
		return new WP_Error( 'php_not_compatible', sprintf( __('The update cannot be installed because CMS %1$s requires PHP version %2$s or higher. You are running version %3$s.'), $wp_version, $required_php_version, $php_version ) );
	elseif ( !$mysql_compat )
		return new WP_Error( 'mysql_not_compatible', sprintf( __('The update cannot be installed because CMS %1$s requires MySQL version %2$s or higher. You are running version %3$s.'), $wp_version, $required_mysql_version, $mysql_version ) );
	...
	// This gives us the ability to install new plugins & themes bundled with future versions of CMS whilst avoiding the re-install upon upgrade issue.
	...
	/**
	 * Fires after CMS core has been successfully updated.
	 *
	 * @since 3.3.0
	 *
	 * @param string $wp_version The current CMS version.
	 */
...
/**
 * Copies a directory from one location to another via the CMS Filesystem Abstraction.
...
/**
 * Redirect to the About CMS page after a successful upgrade.
 *
...
function _redirect_to_about_wordpress( $new_version ) {
	...
	show_message( '<span class="hide-if-no-js">' . sprintf( __( 'Welcome to CMS %1$s. You will be redirected to the About CMS screen. If not, click <a href="%2$s">here</a>.' ), $new_version, 'about.php?updated' ) . '</span>' );
	show_message( '<span class="hide-if-js">' . sprintf( __( 'Welcome to CMS %1$s. <a href="%2$s">Learn more</a>.' ), $new_version, 'about.php?updated' ) . '</span>' );
	echo '</div>';
	?>
