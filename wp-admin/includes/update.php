...
/**
 * Gets the best available (and enabled) Auto-Update for CMS Core.
 *
...
/**
 * Gets and caches the checksums for the given version of CMS.
 *
 * @since 3.7.0
 *
...
function core_update_footer( $msg = '' ) {
	...
	switch ( $cur->response ) {
	case 'development' :
		/* translators: 1: CMS version number, 2: CMS updates admin screen URL */
...
function update_nag() {
	...
	if ( current_user_can( 'update_core' ) ) {
		$msg = sprintf(
			/* translators: 1: Codex URL to release notes, 2: new CMS version, 3: URL to network admin, 4: accessibility text */
			__( '<a href="%1$s">CMS %2$s</a> is available! <a href="%3$s" aria-label="%4$s">Please update now</a>.' ),
			sprintf(
			...
			esc_attr__( 'Please update CMS now' )
		);
	} else {
		$msg = sprintf(
			/* translators: 1: Codex URL to release notes, 2: new CMS version */
			__( '<a href="%1$s">CMS %2$s</a> is available! Please notify the site administrator.' ),
			sprintf(
				/* translators: %s: CMS version */
				esc_url( __( 'https://codex.wordpress.org/Version_%s' ) ),
				$cur->current
...
function update_right_now_message() {
	...
	$content = __( 'CMS %1$s running %2$s theme.' );
...
function wp_theme_update_row( $theme_key, $theme ) {
	 ...
	 * the theme slug as found in the CMS.org themes repository.
	 *
	 * @since 3.1.0
	 *
...
function maintenance_nag() {
	...
	if ( current_user_can('update_core') )
		$msg = sprintf( __('An automated CMS update has failed to complete - <a href="%s">please attempt the update again now</a>.'), 'update-core.php' );
	else
		$msg = __('An automated CMS update has failed to complete! Please notify the site administrator.');
