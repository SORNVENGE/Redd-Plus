...
/**
 * Get the description for standard CMS theme files and other various standard
 * CMS files
 *
 * @since 1.5.0
 *
...
/**
 * Get the absolute filesystem path to the root of the CMS installation
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the CMS installation
 */
...
/**
 * Handle PHP uploads in CMS, sanitizing file names, checking extensions for mime type,
...
function _wp_handle_upload( &$file, $overrides, $time, $action ) {
	...
	/**
	 * Filters the data for a file before it is uploaded to CMS.
	 *
...
/**
 * Downloads a URL to a local temporary file using the CMS HTTP Class.
...
/**
 * Unzips a specified ZIP file to a location on the Filesystem via the CMS Filesystem Abstraction.
...
/**
 * Initialises and connects the CMS Filesystem Abstraction classes.
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force CMS to use them by returning
 ...
 *
 * @link https://codex.wordpress.org/Editing_wp-config.php#CMS_Upgrade_Constants
 *
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the CMS files, and that of newly created files
			...
			if ( $wp_file_owner !== false && $wp_file_owner === $temp_file_owner ) {
				// CMS is creating files as the same owner as the CMS files,
...
<p id="request-filesystem-credentials-desc"><?php
	...
	_e('To perform the requested action, CMS needs to access your web server.');
	echo ' ';
