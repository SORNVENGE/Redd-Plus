<?php
    ...
    if(isset($_POST['old_org_id'])){
        ...
        $code = preg_replace('/\s+/', '_', strtolower($title_en));

        // check old file
		$checkData = $wpdb->get_row( "SELECT * FROM $list_document_type WHERE id != $old_org_id and type_title_en = '$title_en' " );
		...
		if($checkData){ //check doc_type and upload  project_description
            ...
            $data = array(
                'type_title_en' => $title_en,
				'type_title_kh' => $title_kh,
                'type_code' => $code
            );
            ...
            header("Location: ".admin_url()."/admin.php?page=document_type&status=1");
            exit();
        }
        ...
        $edit = true;
        $old_org_id = $_GET['id'];
        $oldInfo = $wpdb->get_row( "SELECT * FROM $list_document_type WHERE id = $old_org_id " );
        ...
        $checkData = $wpdb->get_row( "SELECT * FROM $list_document_type WHERE type_title_en = '$title_en' " );
		...
		if($checkData){ //check doc_type and upload  project_description
            ...
            $format = array('%s','%s');
            $wpdb->insert($list_document_type,$data,$format);
...
<div class="container-fluid">
	...
	<div class="row pt-4 pb-4">
		<?php if($edit){?>
			<h2><a href="?page=document_type">List document type</a> > Edit document type <?php echo $oldInfo->type_title_en;?></h2>
		<?php }else{?>
            <h2><a href="?page=document_type">List document type</a> > Add document type </h2>
		<?php }?>
	</div>
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input type="text" value="<?php if(isset($_POST['title_en'])){ echo $_POST['title_en'];}elseif(isset($oldInfo->type_title_en)){echo $oldInfo->type_title_en ;} ?>" class="form-control require" name="title_en" id="title_en" placeholder="Fill information here...">
                        </div>
                    </div>
					<div class="col-6">
                        <div class="form-group">
                            <label for="exampleFormControlInput1" class="labelinput ">Title khmer*</label>
                            <input type="text" value="<?php if(isset($_POST['title_kh'])){ echo $_POST['title_kh'];}elseif(isset($oldInfo->type_title_kh)){echo $oldInfo->type_title_kh ;} ?>" class="form-control" name="title_kh" id="title_kh" placeholder="Fill information here...">
                        </div>
                    </div>
                </div>
