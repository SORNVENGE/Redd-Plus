<?php
    ...
    if(isset($_POST['old_org_id'])){
        ...
        // check old file
		$checkData = $wpdb->get_row( "SELECT * FROM $organization_category WHERE id != $old_org_id and project_type_en = '$title_en' " );
		...
		if($checkData){ //check doc_type and upload  project_description
            ...
            $data = array(
                'project_type_en' => $title_en,
				'project_type_khmer' => $title_kh
            );
            ...
            header("Location: ".admin_url()."/admin.php?page=project_type&status=1");
            exit();
        }
        ...
        $edit = true;
        $old_org_id = $_GET['id'];
        $oldInfo = $wpdb->get_row( "SELECT * FROM $project_type WHERE id = $old_org_id " );

	}elseif(isset($_POST['title_en'])){
        $title_en = $_POST['title_en'];
		$title_kh = $_POST['title_kh'];
        
        $checkData = $wpdb->get_row( "SELECT * FROM $project_type WHERE project_type_en = '$title_en' " );
		...
		if($checkData){ //check doc_type and upload  project_description
            ...
            $format = array('%s','%s');
            $wpdb->insert($project_type,$data,$format);
...
<div class="container-fluid">
	...
	<div class="row pt-4 pb-4">
		<?php if($edit){?>
			<h2><a href="?page=organizational_category">List project type</a> > Edit project type <?php echo $oldInfo->project_type_en;?></h2>
		<?php }else{?>
            <h2><a href="?page=organizational_category">List project type</a> > Add project type </h2>
		<?php }?>
	</div>
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input type="text" value="<?php if(isset($_POST['title_en'])){ echo $_POST['title_en'];}elseif(isset($oldInfo->project_type_en)){echo $oldInfo->project_type_en ;} ?>" class="form-control require" name="title_en" id="title_en" placeholder="Fill information here...">
                        </div>
                    </div>
					<div class="col-6">
                        <div class="form-group">
                            <label for="exampleFormControlInput1" class="labelinput ">Title khmer*</label>
                            <input type="text" value="<?php if(isset($_POST['title_kh'])){ echo $_POST['title_kh'];}elseif(isset($oldInfo->project_type_khmer)){echo $oldInfo->project_type_khmer ;} ?>" class="form-control" name="title_kh" id="title_kh" placeholder="Fill information here...">
                        </div>
                    </div>
                </div>
